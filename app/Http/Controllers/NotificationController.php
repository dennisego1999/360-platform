...
class NotificationController extends Controller
{
    public function read(ReadNotificationRequest $request, Notification $notification)
    {
        ...
        // Redirect to url
        if ($notification['url']) {
            return redirect($notification['url'])->with('success', trans('spa.notifications.mark_as_read'));
        }

        return redirect()->back()->with('success', trans('spa.notifications.mark_as_read'));
    }
}
